...
import { useAppContext } from "./context/DataContext"; // ← Import this
...
export default function Sidebar() {
  ...
  const menuItems = [
    ...
    { icon: <PackageSearch size={22} />, label: "Counter Requests", path: "/counter-requests" },
    { icon: <Truck size={22} />, label: "Shipments", path: "/shipments" },
  ];

  // These three are admin-only
  ...
  const menuItems2 = [
    { icon: <LogOut size={22} />, label: "Logout", path: "/filters" }, // path unused, but kept
  ];
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col gap-1 mt-8 flex-grow overflow-hidden">
        ...
        {/* Admin-only items – only render if isAdmin */}
